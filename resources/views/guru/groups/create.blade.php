...
<div class="container">
    <div class="flex justify-between items-center mb-4">
        <h1 class="text-2xl font-bold">Buat Kelompok Baru</h1>
    ...
    <div class="bg-white shadow-md rounded-lg p-6">
        <form action="{{ route('dosen.groups.store') }}" method="POST">
            @csrf
            <div class="mb-4">
            ...
            <div class="mb-4">
                ...
                <select class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="problem_id" name="problem_id">
                    ...
                    @foreach ($problems as $problem)
                        <option value="{{ $problem->id }}" {{ old('problem_id') == $problem->id ? 'selected' : '' }}>{{ $problem->judul }}</option>
                    @endforeach
                </select>
            </div>
            <div class="mb-4">
                ...
                <select class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline h-40" id="members" name="members[]" multiple>
                    @foreach ($students as $student)
                        <option value="{{ $student->id }}" {{ in_array($student->id, old('members', [])) ? 'selected' : '' }}>{{ $student->name }}</option>
                    @endforeach
                </select>
